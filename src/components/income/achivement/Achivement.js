...
import { achievementBonousGet } from '../../../api/login/Login'
...
function Achivement() {
  ...
  const getTransitionReport = async (input) => {
    ...
    try {
      const res = await achievementBonousGet(clone)
      // console.log(res);
                ...
                <table id="rechargehistorytbl" className="table table-striped table-bordered table-hover display fixed_headers dataTable" style={{ width: 1250 }} role="grid" aria-describedby="rechargehistorytbl_info">
                  <thead>
                    <tr role="row">
                      ...
                      <th className="sorting">Reword Level</th>
                      <th className="sorting">
                        Status
                      </th>
                    </tr>
                  ...
                  <tbody>
                    {data && data?.map((item, i) => {
                      ...
                      return <tr className="odd" key={item?._id}>
                        ...
                        <td valign="top" className="dataTables_empty">{item?.reward_for_level}</td>
                        <td valign="top" className="dataTables_empty">{item?.pay_status}</td>
